---
title: "R ile Veri Analizinde *Tidyverse* Yaklaşımı"
subtitle: 
author: 
...
# R'ın Modern bir yorumu: Tidyverse 
...
-   Bu kitabın birinci baskısının Türkçe çevirisi için bkz. <https://tr.r4ds.hadley.nz/>
...
`gapminder` Tidyverse jargonunda bir **tibble**'dır. Tibble (ya da `tbl_df`) R veri çerçevesinin daha modern bir yorumu olarak düşünülebilir (detaylar için bkz <https://tibble.tidyverse.org/>).
...
-   Tidyverse meta-paketindeki [`{readr}`](https://readr.tidyverse.org/) paketi verilerin okutulmasına ilişkin çeşitli fonksiyonlar içerir.
...
```{r}
tr_veri <- read_delim("Data/trveri1.csv", delim = ";")
```
...
```{r, warning=FALSE}
tr_veri <- read_tsv("Data/trveri1.txt")
tr_veri
```
...
```{r}
library(readxl)
tr_veri <- read_xlsx("Data/trveri1.xlsx")
tr_veri
```
...
2.  Her gözlem kendi satırında olmalıdır.
3.  Her değer kendi hücresinde olmalıdır.
...
-   Alıştırma: Yukarıdaki veri seti "düzenli" formda mıdır? Değilse nasıl düzenli hale getirilebilir? Cevap:
...
-   Grafiği pdf ya da png formatında saklamak için manuel olarak RStudio Plots penceresinden Export seçilebilir. Alternatif olarak `ggsave()` fonksiyonu kullanılabilir.
...
-   Sürekli bir değişkenin dağılımı betimsel istatistiğin önemli bir parçasıdır. Verilerin daha iyi anlaşılmasında histogram, kutu çizimi, birikimli dağılım fonksiyonu gibi görsel araçlar çok faydalı olabilir.
...
-   Birikimli dağılım fonksiyonunu çizerek normal dağılımla karşılaştırmak isteyebiliriz. Bunun için `geom_qq()` veya `stat_qq()` fonksiyonlarını kullanabiliriz.
...
-   Örnek olarak yukarıda oluşturduğumuz `lowincome_europe` grafiğini siyah-beyaz baskıya uygun hale getirelim:
...
-   `{sf}` (simple features) paketi: <https://github.com/r-spatial/sf>. `ggplot2` fonksiyonları `geom_sf()` ve `coord_sf()` bu paket ile uyumlu çalışır.

-   Örnek olarak Türkiye'nin il düzeyinde bir haritasını çizmek istediğimizi düşünelim.

```{r message=FALSE, warning=FALSE}
library(tidyverse)
load("Data/tr_harita_veri.RData")
```
...
-   `R`'da faktör değişkenleri belirli sayıda öğeleri olan karakter değişkenleridir. Tidyverse'in bir parçası olan `{forcats}` paketi bu tür değişkenlerin kolayca manipülasyonu için tasarlanmıştır.

-   `{forcats}` içindeki fonksiyonlar `fct_` ile başlar. Yaygın olarak kullanılan başlıca fonksiyonlar şunlardır:
...
-   Daha fazla detay için bkz. <https://r4ds.had.co.nz/factors.html> ve <https://forcats.tidyverse.org/>.

# `{stringr}` Paketi ile Karakter Değişkenlerinin İşlenmesi {#stringr}

-   `{stringr}` kütüphanesi karakter değişkenlerinin işlenmesi için tasarlanmış çeşitli fonksiyonlardan oluşur.

-   Bu fonksiyonların hepsi `str_` ile başlar (Console penceresinde `str_` yazarak mevcut fonksiyonların listesini görebilirsiniz). Örneğin,
...
Ç harfi doğru olarak büyük harfe dönüştürüldü ancak Türkçe'de I harfi yoktur. Bunun için `locale = "tr"` opsiyonu kullanılabilir.
...
-   Bir karakterin yerine başka bir karakterin yerleştirilmesi: Örnek olarak Türkçe harfler yerine İngilizce harfleri koymak istediğimizi düşünelim, yani ş yerine s, ı yerine i, vb.
...
-   Örnek: Bir karakter vektöründeki bütün Türkçe harfleri İngilizce harflerle değiştiren bir fonksiyon yazınız.
...
-   Tarih ve saat bilgisini içeren değişkenlerle işlem yapılmasında `{lubridate}` kütüphanesi kullanılabilir.
...
-   Tarih-zaman değişkenleri karakter vektörlerinden hareketle oluşturulabilir. Örneğin
...
-   Sürelerle ilgili daha detaylı bilgi için bkz. <https://r4ds.had.co.nz/dates-and-times.html>
